...
#define SCKeyPath(objc,keyPath) @(((void)objc.keyPath,#keyPath))
 ...
 */
+ (void)sc_swizzleInstanceSelector:(SEL)origSelector
 ...
 *  @param swizzleSelector 现有方法(自己实现方法)
 */
+ (void)sc_swizzleClassSelector:(SEL)origSelector
                swizzleSelector:(SEL)swizzleSelector;
...
+ (void)sc_swizzleClassSelector:(SEL)origSelector swizzleSelector:(SEL)swizzleSelector
{
    // 获取原有方法
...
@implementation UIScrollView (Header)

+ (void)load
{
    [self sc_swizzleInstanceSelector:@selector(setTableHeaderView:) swizzleSelector:@selector(setSc_TableHeaderView:)];
}

// 拦截通过代码设置tableView头部视图
- (void)setSc_TableHeaderView:(UIView *)tableHeaderView
{
    ...
    // 设置tableView头部视图
    [self setSc_TableHeaderView:tableHeaderView];
    
    // 设置头部视图的位置
    UITableView *tableView = (UITableView *)self;
    
    self.sc_headerScaleImageHeight = tableView.tableHeaderView.frame.size.height;
    
}

// 懒加载头部imageView
- (UIImageView *)sc_headerImageView
{
...
- (void)setSc_isInitial:(BOOL)sc_isInitial
{
    objc_setAssociatedObject(self, isInitialKey, @(sc_isInitial),OBJC_ASSOCIATION_ASSIGN);
}

// 属性： sc_headerImageViewHeight
- (void)setSc_headerScaleImageHeight:(CGFloat)sc_headerScaleImageHeight
{
    objc_setAssociatedObject(self, headerImageViewHeight, @(sc_headerScaleImageHeight),OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    // 设置头部视图的位置
    [self setupHeaderImageViewFrame];
}
- (CGFloat)sc_headerScaleImageHeight
{
...
// 属性：sc_headerImage
- (UIImage *)sc_headerScaleImage
{
    return self.sc_headerImageView.image;
}

// 设置头部imageView的图片
- (void)setSc_headerScaleImage:(UIImage *)sc_headerScaleImage
{
    self.sc_headerImageView.image = sc_headerScaleImage;
...
{
    self.sc_headerImageView.frame = CGRectMake(0 , 0, self.bounds.size.width , self.sc_headerScaleImageHeight);
...
- (void)setupHeaderImageView
{
    ...
    if (self.sc_isInitial == NO) {
        [self addObserver:self forKeyPath:SCKeyPath(self, contentOffset) options:NSKeyValueObservingOptionNew context:nil];
        self.sc_isInitial = YES;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    ...
    if (offsetY < 0) {
        ...
        self.sc_headerImageView.frame = CGRectMake(offsetY, offsetY, self.bounds.size.width - offsetY * 2, self.sc_headerScaleImageHeight - offsetY);
...
- (void)dealloc
{
    if (self.sc_isInitial) { // 初始化过，就表示有监听contentOffset属性，才需要移除
        
        [self removeObserver:self forKeyPath:SCKeyPath(self, contentOffset)];
